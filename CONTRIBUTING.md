...
2. Update `version` and `releaseNotes` in `startos/versions/current.ts` in place — leave the filename, the `current` export, and `index.ts` untouched. Add a _new_ version file (and wire it into `index.ts`'s `other` array) only when the bump carries an `up`/`down` migration; git history of `current.ts` preserves prior release notes. See [Versions](https://docs.start9.com/packaging/versions.html).
